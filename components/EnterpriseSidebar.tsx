...
type IconName =
  | 'dashboard' | 'crm' | 'sales' | 'marketing' | 'operations' | 'master' | 'clients' | 'contacts'
  | 'leads' | 'campaigns' | 'activities' | 'reports' | 'analytics' | 'ai' | 'users' | 'settings'
  | 'audit' | 'country' | 'region' | 'branch' | 'industry' | 'supplier' | 'airline' | 'hotel'
  | 'currency' | 'email' | 'templates' | 'sequences' | 'chevron' | 'menu' | 'search' | 'logout'
  | 'profile' | 'help' | 'theme';
    ...
    items: [
      {
        ...
        children: [
          ...
          { label: 'Airline', href: '/admin/airlines', icon: 'airline' },
...
function Icon({ name }: { name: IconName }) {
  const paths: Record<IconName, string[]> = {
    ...
    operations: ['M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8Zm8 4h-2M6 12H4m12.2-5.2-1.4 1.4M9.2 16.8l-1.4 1.4m0-11.4 1.4 1.4m5.6 5.6 1.4 1.4'],
    ...
    airline: ['M2 16l20-8-8 10-3-5-5 3-4 0Z'],
    hotel: ['M4 21V5h16v16M8 21v-6h8v6M8 9h.01M12 9h.01M16 9h.01'],
    currency: ['M12 3v18M8 7h6a3 3 0 0 1 0 6h-4a3 3 0 0 0 0 6h6'],
    email: ['M4 6h16v12H4V6Zm0 0l8 7 8-7'],
    templates: ['M6 3h12v18H6V3Zm3 5h6M9 12h6M9 16h4'],
    sequences: ['M4 7h5v5H4V7Zm11 0h5v5h-5V7ZM4 17h5m6 0h5M9 9h6m-6 10h6'],
    chevron: ['m9 6 6 6-6 6'],
    menu: ['M4 6h16M4 12h16M4 18h16'],
    search: ['m21 21-4.35-4.35M10.5 18a7.5 7.5 0 1 1 0-15 7.5 7.5 0 0 1 0 15Z'],
    logout: ['M10 17l5-5-5-5M15 12H3m8-8h7a3 3 0 0 1 3 3v10a3 3 0 0 1-3 3h-7'],
    profile: ['M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm-7 8a7 7 0 0 1 14 0'],
    help: ['M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0-5h.01M10 9a2 2 0 1 1 3.3 1.5c-.8.6-1.3 1.1-1.3 2.5'],
    theme: ['M12 3a9 9 0 0 0 0 18 7 7 0 0 1 0-18Z']
  };

  return (
    <svg className="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8">
      {paths[name].map((path) => <path key={path} d={path} strokeLinecap="round" strokeLinejoin="round" />)}
    </svg>
  );
}
...
function SidebarItem({ item, collapsed, level = 0, onNavigate }: { item: NavItem; collapsed: boolean; level?: number; onNavigate?: () => void }) {
  ...
  return (
    <Link
      ...
      onClick={onNavigate}
      className={`group relative flex h-11 items-center gap-3 rounded-xl px-4 text-sm font-semibold transition duration-200 hover:translate-x-0.5 ${
        active ? 'bg-emerald-100 text-emerald-700' : item.ai ? 'bg-gradient-to-r from-emerald-700 to-orange-500 text-white shadow-sm' : 'text-slate-700 hover:bg-emerald-50 hover:text-emerald-700'
      } ${level > 0 && !collapsed ? 'ml-4' : ''}`}
    >
      {active ? <span className="absolute left-0 top-2 h-7 w-1 rounded-r-full bg-emerald-700" /> : null}
      <Icon name={item.icon} />
...
function SidebarGroup({ group, collapsed, query, onNavigate }: { group: NavGroup; collapsed: boolean; query: string; onNavigate?: () => void }) {
  ...
  return (
    <section className="space-y-2">
      {!collapsed ? <p className="px-4 text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">{group.title}</p> : null}
        ...
        {visibleItems.map((item) => {
          ...
          return (
            <div key={item.label}>
              <button
                title={collapsed ? item.label : undefined}
                className={`group relative flex h-11 w-full items-center gap-3 rounded-xl px-4 text-left text-sm font-semibold transition duration-200 hover:translate-x-0.5 ${isItemActive(pathname, item) ? 'bg-emerald-100 text-emerald-700' : 'text-slate-700 hover:bg-emerald-50 hover:text-emerald-700'}`}
                onClick={() => setOpenGroups((current) => ({ ...current, [item.label]: !itemOpen }))}
                type="button"
              >
                {isItemActive(pathname, item) ? <span className="absolute left-0 top-2 h-7 w-1 rounded-r-full bg-emerald-700" /> : null}
                <Icon name={item.icon} />
                {!collapsed ? <span className="truncate">{item.label}</span> : null}
...
export function EnterpriseSidebar({ user }: { user: SidebarUser | null }) {
  ...
  const content = (
    <div className={`flex h-full flex-col border-r border-slate-200 bg-white shadow-[0_0_25px_rgba(15,23,42,0.04)] transition-all duration-300 ${collapsed ? 'w-[72px]' : 'w-[280px]'}`}>
      <div className="flex h-[72px] shrink-0 items-center justify-between gap-2 border-b border-slate-100 px-4">
        ...
        <button className="grid h-9 w-9 shrink-0 place-items-center rounded-xl text-slate-500 hover:bg-emerald-50 hover:text-emerald-700" onClick={() => setCollapsed((value) => !value)} type="button" aria-label="Toggle sidebar">
          <Icon name="menu" />
        </button>
      </div>
        ...
        <div className="px-4 py-3">
          ...
          {quickMatches.length ? (
            <div className="mt-2 rounded-xl border border-slate-100 bg-white p-2 shadow-sm">
              {quickMatches.map((item) => <Link key={`${item.href}-${item.label}`} className="block rounded-lg px-3 py-2 text-xs font-bold text-slate-600 hover:bg-emerald-50" href={item.href}>{item.label}</Link>)}
            </div>
          ) : null}
        </div>
      ) : null}
      ...
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-3 py-4">
        {!collapsed ? (
          <section className="space-y-2">
            <p className="px-4 text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">Recent</p>
            <div className="grid gap-1">{recentItems.map((item) => <SidebarItem key={item.label} item={item} collapsed={collapsed} onNavigate={() => setMobileOpen(false)} />)}</div>
          </section>
        ) : null}
      ...
      <div className="shrink-0 border-t border-slate-100 p-3">
        <div className={`relative rounded-2xl border border-slate-200 bg-slate-50 p-3 ${collapsed ? 'grid place-items-center' : ''}`}>
          <div className="flex items-center gap-3">
            {user?.picture ? <img src={user.picture} alt="Profile" className="h-10 w-10 rounded-full object-cover" /> : <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-slate-950 text-xs font-black text-orange-400">{initials(user?.name, user?.email)}</div>}
            {!collapsed ? <div className="min-w-0"><p className="truncate text-sm font-black text-slate-950">{user?.name || 'Satguru User'}</p><p className="truncate text-xs font-semibold capitalize text-slate-500">{labelRole(user?.role)} · {user?.branch || 'Pune HO'}</p></div> : null}
          </div>
          {!collapsed ? <div className="mt-3 grid grid-cols-2 gap-2"><Link href="/profile" className="rounded-xl bg-white px-3 py-2 text-center text-xs font-black text-slate-700 hover:bg-emerald-50">Profile</Link><Link href="/api/auth/logout" className="rounded-xl bg-white px-3 py-2 text-center text-xs font-black text-slate-700 hover:bg-emerald-50">Logout</Link></div> : null}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <>
      <button className="fixed bottom-5 left-5 z-50 grid h-12 w-12 place-items-center rounded-2xl bg-emerald-700 text-white shadow-xl lg:hidden" onClick={() => setMobileOpen(true)} type="button" aria-label="Open sidebar"><Icon name="menu" /></button>
      <aside className="hidden h-[calc(100vh-72px)] shrink-0 lg:block">{content}</aside>
      {mobileOpen ? <div className="fixed inset-0 z-[70] bg-slate-950/40 backdrop-blur-sm lg:hidden"><div className="h-full w-[280px]">{content}</div><button className="absolute right-4 top-4 rounded-xl bg-white px-4 py-2 text-sm font-black text-slate-900" onClick={() => setMobileOpen(false)} type="button">Close</button></div> : null}
    </>
  );
}
