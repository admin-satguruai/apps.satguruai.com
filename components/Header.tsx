...
export function Header({ user = null }: { user?: HeaderUser | null }) {
  ...
  return (
    <header className="fixed left-[var(--satguru-sidebar-width,72px)] right-0 top-0 z-50 h-[64px] border-b border-emerald-900/10 bg-gradient-to-r from-emerald-50/95 via-white/95 to-orange-50/95 shadow-sm shadow-slate-900/5 backdrop-blur-2xl transition-all duration-300 max-lg:left-0">
      <div className="flex h-full w-full items-center gap-4 px-4 sm:px-5 lg:px-6">
        ...
        <div className="hidden min-w-0 flex-1 justify-start md:flex">
          <div className="relative w-full max-w-3xl">
            ...
            <input
              className="h-10 w-full rounded-2xl border border-slate-200 bg-white/90 pl-11 pr-24 text-sm font-semibold text-slate-400 outline-none shadow-inner shadow-slate-100/80 ring-1 ring-white/70"
        ...
        <div className="ml-auto flex shrink-0 items-center gap-3">
          ...
          <div className="relative">
            <button
              className="flex max-w-[280px] items-center gap-3 rounded-2xl border border-slate-200 bg-white/95 px-3 py-2 shadow-sm shadow-slate-900/5 transition hover:bg-emerald-50"
